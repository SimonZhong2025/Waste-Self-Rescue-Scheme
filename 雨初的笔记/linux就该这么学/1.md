...
+ `firewall-cmd` 命令行配置
  ...
  + 配置成 `runtime` 模式是当前生效，重启后失效。 `-permanent` 是当前不生效，重启后再生效。使用 `firewall-cmd --reload` 可以立即生效，不用再重启生效。
  + `--query-service` 可以查询某个服务的状态，比如 `firewall-cmd --zone=public --query-service=ssh`
  + 