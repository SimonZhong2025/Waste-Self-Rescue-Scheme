...
+ 可以用 `||sh` 来绕过不能直接指向 `sh` 所在地址的情况。如
  ...
  ![image-20201024115238896](https://cdn.jsdelivr.net/gh/smallzhong/picgo-pic-bed/image-20201024115238896.png)
...
![image-20201023112352043](https://cdn.jsdelivr.net/gh/smallzhong/picgo-pic-bed/image-20201023112352043.png)

![image-20201023192848147](https://cdn.jsdelivr.net/gh/smallzhong/picgo-pic-bed/image-20201023192848147.png)



## 大体

![image-20201023164054310](https://cdn.jsdelivr.net/gh/smallzhong/picgo-pic-bed/image-20201023164054310.png)
...
+ 可以通过 `prev size` 得到上一个 `chunk` 的起始位置，不断通过 `prev size` 得到上一个 `chunk` 的起始位置，这其实达到了链表的效果。（这叫做 **物理链表** ）
  ...
  ![image-20201023172144116](https://cdn.jsdelivr.net/gh/smallzhong/picgo-pic-bed/image-20201023172144116.png)
...
+ 一个 `chunk` 最小是8个字节（只有头部控制信息），而且每一个 `chunk` 都是 `8` 的整数倍，即其 `size` 一定是8的倍数。
  ...
  ![image-20201023162607646](https://cdn.jsdelivr.net/gh/smallzhong/picgo-pic-bed/image-20201023162607646.png)

  因为如果这个 `size` 全部用来存放这个 `chunk` 的大小，其末**3bit**永远为0（一定是8的倍数），所以这3个bit用来存放控制信息（A/M/P）。

  + `size` 对应的大小包括 `prev size` 和 `size` 这8个字节的大小。

+ ![image-20201023192405094](https://cdn.jsdelivr.net/gh/smallzhong/picgo-pic-bed/image-20201023192405094.png)

+ `unsorted bin` 可以类比为一个缓冲区，之后会分配到对应的 `bin` 中。

### free chunk

​	![image-20201023162728757](https://cdn.jsdelivr.net/gh/smallzhong/picgo-pic-bed/image-20201023162728757.png)
...
+ `prev size` 记录的是前一个 `free chunk` 的大小。如果前一个 `chunk` 不是 `free chunk` ，那么这个字段就没有用了，操作系统也可能会将这8个字节分配给前一个 `chunk` 。
  ...
  ![image-20201023171734376](https://cdn.jsdelivr.net/gh/smallzhong/picgo-pic-bed/image-20201023171734376.png)



## fastbin free chunk

![image-20201023163833004](https://cdn.jsdelivr.net/gh/smallzhong/picgo-pic-bed/image-20201023163833004.png)

+ 最小的 `chunk` 被 `free` 掉之后会放到 `fastbin` 里面。

+ `fastbin` 的结构跟拉链法有点像，这是一个逻辑链表（通过指针连接在一起）

  ![image-20201023185838590](https://cdn.jsdelivr.net/gh/smallzhong/picgo-pic-bed/image-20201023185838590.png)

+ `fd` 指针真实指向的位置和图中一致，指向的是下一个 `chunk` 的 `chunk` 头的位置。

+ ![image-20201023191846695](https://cdn.jsdelivr.net/gh/smallzhong/picgo-pic-bed/image-20201023191846695.png)



### small bin free chunk

+ ![image-20201023192156117](https://cdn.jsdelivr.net/gh/smallzhong/picgo-pic-bed/image-20201023192156117.png)



### large bin free chunk

+ ![image-20201023192549925](https://cdn.jsdelivr.net/gh/smallzhong/picgo-pic-bed/image-20201023192549925.png)
...
+ `double free`
  ...
  ![image-20201023212110024](https://cdn.jsdelivr.net/gh/smallzhong/picgo-pic-bed/image-20201023212110024.png)
...
+ 输入 `fastbin` 可以查看  `fastbin free chunk`
  ...
  ![image-20201023165335679](https://cdn.jsdelivr.net/gh/smallzhong/picgo-pic-bed/image-20201023165335679.png)

+ 输入 `heap` 可以查看堆相关的信息。

  ![image-20201023171026047](https://cdn.jsdelivr.net/gh/smallzhong/picgo-pic-bed/image-20201023171026047.png)
...
+ 如果想要对开启了 `PIE` 的程序进行调试，可以根据其偏移下断点，如
  ...
  ![image-20201023211026169](https://cdn.jsdelivr.net/gh/smallzhong/picgo-pic-bed/image-20201023211026169.png)
