...
+ 这里我们不需要二分来做。我们可以直接通过 `kruskal` 来做。因为克鲁斯卡尔的话会从最短的边开始到最长的边。（其实这是类似克鲁斯卡尔，并不是克鲁斯卡尔吧。。只是用了排序和并查集跟克鲁斯卡尔有点像。）这里当我们联通块数小于卫星数 $k$ 的时候，说明已经找到了结果。而答案就是最后一条边的长度，因为最后一条边是最大d
