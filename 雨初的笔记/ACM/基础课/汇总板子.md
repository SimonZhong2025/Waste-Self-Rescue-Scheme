...
**被划分成[l ,mid]\[mid + 1, r]就是l = mid + 1，[l ,mid - 1]\[mid, r]就是l = mid**

**总得有一个 `+1` ，要么就是 `l = mid + 1` ，要么就是 `l + r + 1 >> 1` ** 
...
+ 差分数组的前缀和得出原数组。比如题目中要给一段区间中的数加上同一个数，可以用到差分的方法。 `q[l] += t, q[r + 1] -=t` 这样求出来的前缀和在 `[l, r]` 这个区间内每个数都会加上1，但是不会影响到其他的数。 
...
![image-20201031100246096](https://raw.githubusercontent.com/smallzhong/new-picgo-pic-bed/main/image-20201031100246096.png)
...
![image-20201031100417192](https://raw.githubusercontent.com/smallzhong/new-picgo-pic-bed/main/image-20201031100417192.png)
...
![image-20201031100438941](https://cdn.jsdelivr.net/gh/smallzhong/new-picgo-pic-bed@master/image-20201031100438941.png)
...
+ `prim` 中的 `dist[t]` 表示的是**这个点到集合的距离**，而 `dijkstra` 算法中的 `dist[t]` 表示的是这个点到起点的距离。所以在 `prim` 算法中每一次找到新的点 **t点** 之后就要将整个图里面所有的点到集合的距离更新。即 `dist[j] = min(dist[j], g[t][j])` 。
  ...
  ![image-20201026100737145](https://cdn.jsdelivr.net/gh/smallzhong/new-picgo-pic-bed@master//image-20201026100737145.png)
